...
- (void)openInInAppBrowser:(NSURL*)url withOptions:(NSString*)options
{
    ...
    self.fetchCookieKey = browserOptions.fetchcookiekey;
    self.fetchCookieURL = browserOptions.fetchcookieurl;
...
{
    if(self.fetchCookieURL != nil && self.fetchCookieKey != nil)
    {
        NSHTTPCookieStorage *storage = [NSHTTPCookieStorage sharedHTTPCookieStorage];
        NSArray *cookiesArray = [storage cookies];
        for (NSHTTPCookie *cookie in cookiesArray) {
            NSLog(@"Cookie Value = %@",[cookie value]);
            if ([[cookie name] isEqualToString:self.fetchCookieKey]) {
                
                if([cookie value] != nil)
                {
                    NSURL *newURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@?%@=%@",self.fetchCookieURL,self.fetchCookieKey,[cookie value]]];
